...
2. **并行分词+计数** — 5000 行/chunk，10 chunk/batch，Pool 多进程并行（macOS 用 `spawn` 避免死锁）。三种模式：jieba 分词、预分词计数、预分词词性标注。每个 batch 带 600s 超时保护，单行文件走字节级分流
...
67 个用例覆盖 segment.py、pipeline.py、merge、CLI、预分词、集成测试。
